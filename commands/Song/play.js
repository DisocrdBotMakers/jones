...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
      ...
      if (!channel) return message.channel.send(`:x: **You need to be in a voice channel to use this command.**`);
      //send error if member is Deafed
      if(message.member.voice.selfDeaf) return message.channel.send(`:x: **You cannot use this command while being deafened.**`);
      ...
      //ELSE SEND CLIENT INFO
    } else if(message.content.includes("http")){
      //send searching
      message.channel.send(`<:promusic:826519647347539990> **Searching** :mag_right: \`${args.join(" ")}\``);
      //play the song from our playermanager
      playermanager(client, message, args, `play:http`);
    } else {
      //send searching
      message.channel.send(`<:youtube:826100274095194132> **Searching** :mag_right: \`${args.join(" ")}\``);
      //play the song from our playermanager
